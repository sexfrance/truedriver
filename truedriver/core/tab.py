...
class Tab(Connection):
    ...
    async def query_selector_all(
        ...
        if not _node:
            ...
            if current_frame:
                # Get the document for the current frame
                frame_tree = await self.send(cdp.page.get_frame_tree())
                
                def find_frame_doc(frame_tree_node: cdp.page.FrameTree) -> Any:
                    if frame_tree_node.frame.id_ == current_frame.id_:
                    ...
                    if frame_tree_node.child_frames:
                        for child in frame_tree_node.child_frames:
                            result = find_frame_doc(child)
                            if result:
                                return result
                    return None
                
                frame_node = find_frame_doc(frame_tree)
                if frame_node:
                    # Get the DOM document for this frame
                    doc = await self.send(cdp.dom.get_document(-1, True))
                    # Find the iframe element and use its content document
                    iframe_nodes = await self.send(cdp.dom.query_selector_all(doc.node_id, "iframe"))
                    for iframe_node_id in iframe_nodes:
                        iframe_node = await self.send(cdp.dom.describe_node(iframe_node_id))
                        if (hasattr(iframe_node, 'content_document') and 
                            iframe_node.content_document and
                            hasattr(iframe_node.content_document, 'base_url')):
                            # Check if this iframe matches our current frame
                            if current_frame.url in iframe_node.content_document.base_url or \
                               iframe_node.content_document.base_url in current_frame.url:
                                doc = iframe_node.content_document
                                break
                    else:
                        # Fallback to main document if frame document not found
                        doc = await self.send(cdp.dom.get_document(-1, True))
                else:
                    doc = await self.send(cdp.dom.get_document(-1, True))
            else:
    ...
    async def evaluate(
        ...
        # Get execution context for current frame
        execution_context_id = self._get_execution_context_for_evaluate()
    ...
    async def get_frames(self) -> List[cdp.page.Frame]:
        ...
        def collect_frames(frame_tree_node: cdp.page.FrameTree) -> None:
    ...
    async def find_frame_by_url(self, url_pattern: str) -> Optional[cdp.page.Frame]:
        """
        Find a frame by URL pattern.
        
        :param url_pattern: URL pattern to search for (supports regex)
        :type url_pattern: str
        :return: Frame if found, None otherwise
        :rtype: Optional[cdp.page.Frame]
        """
    ...
    async def find_frame_by_name(self, name: str) -> Optional[cdp.page.Frame]:
        """
        Find a frame by name attribute.
        
        :param name: Frame name to search for
        :type name: str
        :return: Frame if found, None otherwise
        :rtype: Optional[cdp.page.Frame]
        """
    ...
    async def switch_to_frame(self, frame: Union[cdp.page.Frame, str, Element]) -> None:
        """
        Switch to a specific frame for subsequent operations.
        
        :param frame: Frame to switch to - can be Frame object, frame ID string, frame name, URL pattern, or iframe Element
        :type frame: Union[cdp.page.Frame, str, Element]
        """
        if isinstance(frame, str):
            # Try to find frame by name first, then by URL pattern
            found_frame = await self.find_frame_by_name(frame)
            if not found_frame:
                found_frame = await self.find_frame_by_url(frame)
            if not found_frame:
                raise ValueError(f"Frame not found: {frame}")
            frame = found_frame
        elif isinstance(frame, element.Element):
            # Get the frame ID from the iframe element
            frame_id = await frame.get_attribute("id")
            if not frame_id:
                # Try to get the frame by matching the iframe src with frame URLs
                src = await frame.get_attribute("src")
                if src:
                    found_frame = await self.find_frame_by_url(src)
                    if found_frame:
                        frame = found_frame
                    else:
                        raise ValueError("Could not determine frame from iframe element")
                else:
                    raise ValueError("Iframe element has no id or src attribute")
            else:
                found_frame = await self.find_frame_by_name(frame_id)
                if found_frame:
                    frame = found_frame
                else:
                    raise ValueError(f"Frame not found with id: {frame_id}")
        
        if not isinstance(frame, cdp.page.Frame):
            raise ValueError("Invalid frame type provided")
        
        # Store the current frame context
        self._current_frame_id = frame.id_
        
        # Enable Runtime domain to get execution contexts
        await self.send(cdp.runtime.enable())
        
        # Get execution contexts and find the one for this frame
        try:
            contexts = await self.send(cdp.runtime.get_execution_contexts())
            frame_context = None
            
            for context in contexts:
                if hasattr(context, 'frame_id') and context.frame_id == frame.id_:
                    frame_context = context
                    break
                # Some contexts might not have frame_id, so we check the name/origin
                elif (hasattr(context, 'origin') and context.origin == frame.security_origin) or \
                     (hasattr(context, 'name') and frame.name and context.name == frame.name):
                    frame_context = context
                    break
            
            if frame_context:
                self._current_execution_context_id = frame_context.id_
            else:
                # If no specific context found, we'll use the frame ID for subsequent operations
                self._current_execution_context_id = None
                
        except Exception as e:
            logger.warning(f"Could not get execution context for frame {frame.id_}: {e}")
            self._current_execution_context_id = None
    ...
    async def switch_to_frame_by_index(self, index: int) -> None:
        """
        ...
        if index < 0 or index >= len(frames):
            raise IndexError(f"Frame index {index} out of range. Available frames: {len(frames)}")
        
        await self.switch_to_frame(frames[index])

    async def get_current_frame(self) -> Optional[cdp.page.Frame]:
        """
        Get the currently active frame.
        
        :return: Current frame or None if in main frame
        :rtype: Optional[cdp.page.Frame]
        """
        if not hasattr(self, '_current_frame_id') or self._current_frame_id is None:
            return None
        
        frames = await self.get_frames()
        for frame in frames:
            if frame.id_ == self._current_frame_id:
                return frame
        return None

    def _get_execution_context_for_evaluate(self) -> Optional[cdp.runtime.ExecutionContextId]:
        """
        Get the execution context ID for the current frame.
        
        :return: Execution context ID or None for main frame
        :rtype: Optional[cdp.runtime.ExecutionContextId]
        """
        if hasattr(self, '_current_execution_context_id'):
            return self._current_execution_context_id
        return None
