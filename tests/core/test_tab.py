...
async def test_set_user_agent_sets_navigator_values(browser: td.Browser) -> None:
...
async def test_set_user_agent_defaults_existing_user_agent(browser: td.Browser) -> None:
    tab = browser.main_tab
    assert tab is not None
...
async def test_find_finds_element_by_text(browser: td.Browser) -> None:
...
async def test_find_times_out_if_element_not_found(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
...
async def test_select(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
...
async def test_xpath(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
...
async def test_add_handler_type_event(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))

    async def request_handler_1(event: td.cdp.network.RequestWillBeSent) -> None:
        pass

    async def request_handler_2(event: td.cdp.network.RequestWillBeSent) -> None:
        pass

    assert len(tab.handlers) == 0

    tab.add_handler(td.cdp.network.RequestWillBeSent, request_handler_1)

    tab.add_handler(td.cdp.network.RequestWillBeSent, request_handler_2)

    assert len(tab.handlers) == 1
    assert len(tab.handlers[td.cdp.network.RequestWillBeSent]) == 2
    assert tab.handlers[td.cdp.network.RequestWillBeSent] == [
        request_handler_1,
        request_handler_2,
    ]


async def test_add_handler_module_event(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
    ...
    tab.add_handler(td.cdp.network, request_handler)

    assert len(tab.handlers) == 27


async def test_remove_handlers(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))

    async def request_handler(event: td.cdp.network.RequestWillBeSent) -> None:
        pass

    tab.add_handler(td.cdp.network.RequestWillBeSent, request_handler)
    assert len(tab.handlers) == 1
...
async def test_remove_handlers_specific_event(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
    ...
    tab.remove_handlers(
        td.cdp.network.RequestWillBeSent,
    )
    assert len(tab.handlers) == 0


async def test_remove_specific_handler(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
    ...
    tab.remove_handlers(td.cdp.network.RequestWillBeSent, request_handler_1)
    assert len(tab.handlers) == 1
    assert len(tab.handlers[td.cdp.network.RequestWillBeSent]) == 1


async def test_remove_handlers_without_event(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
...
async def test_wait_for_ready_state(browser: td.Browser) -> None:
    tab = await browser.get(sample_file("groceries.html"))
...
async def test_expect_request(browser: td.Browser) -> None:
    tab = browser.main_tab
    assert tab is not None
    ...
    async with tab.expect_request(sample_file("groceries.html")) as request_info:
        ...
        assert type(req) is td.cdp.network.RequestWillBeSent
        assert type(req.request) is td.cdp.network.Request
...
async def test_expect_response(browser: td.Browser) -> None:
    tab = browser.main_tab
    assert tab is not None
    ...
    async with tab.expect_response(sample_file("groceries.html")) as response_info:
        ...
        assert type(resp) is td.cdp.network.ResponseReceived
        assert type(resp.response) is td.cdp.network.Response
...
async def test_expect_download(browser: td.Browser) -> None:
    tab = browser.main_tab
    assert tab is not None
    ...
    async with tab.expect_download() as download_ex:
        ...
        assert type(download) is td.cdp.browser.DownloadWillBegin
        assert download.url is not None


async def test_intercept(browser: td.Browser) -> None:
    tab = browser.main_tab
    assert tab is not None
